module.exports = async (sock, msg, comando, args, db, salvarDB) => {
    ...
    // Função auxiliar para contar quantos usuários possuem uma determinada raridade no grupo
    ...
    switch (comando) {
        ...
        case 'minerar':
            ...
            if (sorte > 0.4) {
                const ganhoMin = Math.floor(Math.random() * 101) + 50; // 50 a 150
                u.golds += ganhoMin;
                salvarDB(db);
                ...
                const perdaMin = Math.floor(Math.random() * 41) + 20; // 20 a 60
        ...
        case 'assaltar':
            const alvoAssalto = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
            ...
            if (!db.usuarios[alvoAssalto]) db.usuarios[alvoAssalto] = { golds: 100, banco: 0, escudo: false };
            let vitima = db.usuarios[alvoAssalto];

            if (vitima.golds < 50) return sock.sendMessage(from, { text: "💧 Esse membro está muito pobre, não vale a pena assaltá-lo. O crime não compensa tanto assim!" }, { quoted: msg });

            if (vitima.escudo) {
                vitima.escudo = false;
                u.golds = Math.max(0, u.golds - 300); // Multa pesada
                salvarDB(db);
            ...
            if (Math.random() > 0.5) {
                const roubado = Math.floor(vitima.golds * 0.3); // Rouba 30% do dinheiro em mãos
        ...
        case 'banco':
            ...
            if (acao === 'depositar') {
                ...
                if (u.banco < valor) return sock.sendMessage(from, { text: "❌ Você não tem toda essa quantia guardada no banco!" }, { quoted: msg });
                u.banco -= valor;
                u.golds += valor;
                salvarDB(db);
        ...
        case 'pagar':
            const recebedor = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
            ...
            u.golds -= valorPg;
            db.usuarios[recebedor].golds += valorPg;
            salvarDB(db);
        ...
        case 'comprar':
            ...
            u.data_expiracao = Date.now() + 604800000; // Validade rígida estrita de 1 semana
            salvarDB(db);
